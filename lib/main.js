...
function isNode() {
	return (
		...
		(
			// Node < v7
			(nativeClass( Global ) === '[object global]' || // Node >= v7 (https://github.com/nodejs/node/issues/9274)
		...
		isString( proc.versions.node ) && // `process.release` was added in Node v3.0.0 via io.js:
		(// eslint-disable-line n/no-unsupported-features/node-builtins
		typeof proc.release === 'undefined' || // Check for a `release` property:
		// eslint-disable-line n/no-unsupported-features/node-builtins
		(// eslint-disable-line n/no-unsupported-features/node-builtins
		isObject( proc.release ) && // eslint-disable-line n/no-unsupported-features/node-builtins

		// Check for a `name` property:
		isString( proc.release.name ) && // Check that the release name contains either `node` or `io.js` (in Node.js/io.js, the release name is read-only):
